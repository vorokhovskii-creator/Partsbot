...
Каждый вариант цены: ЦЕНА*КОЛ-ВОПоставщикИлиБренд (если кол-во=1, то без *1)
- ЦЕНА — розничная цена в рублях, только цифры без пробелов и знака ₽
- КОЛ-ВО — количество (если не указано — 1, если 1 — не пиши *1)
...
фильтр топливный\t1190LYNX\\1230Bosch\\1310FILTRON\\2020EUROREPAR\\3230MANN

ПРАВИЛА:
- Если кол-во = 1, НЕ пиши *1 (только цена и поставщик)
- ОКРУГЛЕНИЕ ЦЕН: если цена >= 20 рублей — округляй до ближайшего кратного 50 (в большую если >= 25, в меньшую если < 25). Если цена < 20 рублей — округляй в меньшую сторону до целого
...
ПРАВИЛА ГРУППИРОВКИ ЗАПЧАСТЕЙ:
- Ключ группировки — ТОЧНОЕ и ПОЛНОЕ название запчасти, включая любые уточнения
- Слова "передние", "задние", "передн.", "задн.", "передние к-кт", "задние к-кт" и любые другие уточнения являются НЕОТЪЕМЛЕМОЙ частью названия
- "Колодки тормозные передние к-кт" и "Колодки тормозные задние к-кт" — это РАЗНЫЕ запчасти, выводи их строго отдельными строками
- Если название не содержит уточнения (например, просто "Колодки тормозные дисковые") — выводи такие строки как отдельную позицию, не пытайся угадать к какой группе они относятся и не объединяй их с позициями, у которых уточнение есть
- Никогда не додумывай и не дополняй название запчасти уточнениями, которых нет в исходных данных

ПРИМЕР — ПРАВИЛЬНО (скриншот содержит передние, задние и без уточнения):
колодки тормозные задние дисковые к-кт\t5190*1Brembo\\6339*1Zimmermann\\3205*1TRW
колодки тормозные передние к-кт\t5842*1Brembo\\6452*1TRW
колодки тормозные дисковые\t6338*1Zimmermann\\6738*1Zimmermann

ПРИМЕР — НЕПРАВИЛЬНО (нельзя смешивать позиции с разными уточнениями):
колодки тормозные\t5190*1Brembo\\6339*1Zimmermann\\5842*1Brembo\\6452*1TRW`;
